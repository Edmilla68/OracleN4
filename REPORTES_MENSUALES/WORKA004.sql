DEFINE V_DATEINI = TO_DATE('2019-09-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS');
DEFINE V_DATEFIN = TO_DATE('2019-10-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS');
    ...
    ufv.Last_Pos_Name AS "Position",
     initcap(trim(SUBSTR(ufv.transit_state ,5,length(ufv.transit_state)))) AS "T-State", --ufv.transit_state AS "T-State",
     initcap(trim(SUBSTR(ufv.visit_state ,2,length(ufv.visit_state)))) AS "V-State",     --ufv.visit_state AS "V-State",
    u.flex_string10 AS "Deposito Temporal",
    U.flex_string05 AS "Autorizacion SUNAT"
    ...
    WHERE U.CATEGORY = 'IMPRT' -- AND u.freight_kind = 'FCL'
    AND UFV.TIME_IN >=  &V_DATEINI 
    AND UFV.TIME_IN <=  &V_DATEFIN
