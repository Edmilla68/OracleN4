...
TO_CHAR(VVD.PUBLISHED_ETA, 'YYYYMMDD') as arrival_date,
TO_CHAR(VVD.PUBLISHED_ETD, 'YYYYMMDD') as departure_date,
CASE WHEN LENGTH( RUC_ORI.PLACE_NAME ||' , '|| CNTRY_ORI.CNTRY_NAME)  <= 3 THEN '' ELSE RUC_ORI.PLACE_NAME ||' , '|| CNTRY_ORI.CNTRY_NAME END AS Origin_Country,
CASE WHEN LENGTH( RUC_BQO.PLACE_NAME ||' , '|| CNTRY_BQO.CNTRY_NAME)  <= 3 THEN '' ELSE RUC_BQO.PLACE_NAME ||' , '|| CNTRY_BQO.CNTRY_NAME END AS port_of_landing,
CASE WHEN LENGTH( RUC_DES.PLACE_NAME ||' , '|| CNTRY_DES.CNTRY_NAME)  <= 3 THEN '' ELSE RUC_DES.PLACE_NAME ||' , '|| CNTRY_DES.CNTRY_NAME END AS destination_country,
CASE WHEN LENGTH( RUC_BQD.PLACE_NAME ||' , '|| CNTRY_BQD.CNTRY_NAME)  <= 3 THEN '' ELSE RUC_BQD.PLACE_NAME ||' , '|| CNTRY_BQD.CNTRY_NAME END AS port_of_unlading,
--CASE WHEN LENGTH( RUC_BQO.PLACE_NAME ||' , '|| CNTRY_BQO.CNTRY_NAME AS port_of_landing,
--CASE WHEN LENGTH( RUC_DES.PLACE_NAME ||' , '|| CNTRY_DES.CNTRY_NAME AS destination_country,
--CASE WHEN LENGTH( RUC_BQD.PLACE_NAME ||' , '|| CNTRY_BQD.CNTRY_NAME AS port_of_unlading,
CASE RET.NOMINAL_LENGTH WHEN 'NOM40' THEN 2 ELSE 1 END  AS TEU,
'' as Goods_Value_USD,
'' as notifier_name,
BL.CUSTDFF_BL_FLEX_STRING13 as Consignee_name,
RFS_SH.ID as shipper_name,
BL.CUSTDFF_BL_FLEX_STRING02 AS item_description,
...
AND UFV.FCY_GKEY = 3575349497;
--AND VS.name IS NULL; --ARGENTINA
